...
ggHiNtuplizer::ggHiNtuplizer(const edm::ParameterSet& ps)
  ...
  if (doSuperClusters_) {
    barrelSCToken_ = consumes<reco::SuperClusterCollection>(ps.getParameter<edm::InputTag>("superClustersEB"));
    endcapSCToken_ = consumes<reco::SuperClusterCollection>(ps.getParameter<edm::InputTag>("superClustersEE"));
  }
  if (doElectrons_) {
  ...
  if (doRecHitsEB_ || doEleEReg_) {
    recHitsEB_ =
        consumes<EcalRecHitCollection>(ps.getUntrackedParameter<edm::InputTag>("recHitsEB", edm::InputTag("ecalRecHit", "EcalRecHitsEB")));
  }
  if (doRecHitsEE_ || doEleEReg_) {
    recHitsEE_ =
        consumes<EcalRecHitCollection>(ps.getUntrackedParameter<edm::InputTag>("recHitsEE", edm::InputTag("ecalRecHit", "EcalRecHitsEE")));
  }
  ...
  if (doPfIso_) {
    ...
    if (removePhotonPfIsoFootprint_) {
      particleBasedIsolationPhoton_ =
          mayConsume<edm::ValueMap<std::vector<reco::PFCandidateRef>>>(ps.getParameter<edm::InputTag>("particleBasedIsolationPhoton"));
    }
  }
...
void ggHiNtuplizer::fillGenParticles(const edm::Event& e) {
  ...
  for (auto p = genParticlesHandle->begin(); p != genParticlesHandle->end(); ++p) {
    ...
    bool isHeavy = (p->pdgId() == 23 || abs(p->pdgId()) == 24 || p->pdgId() == 25 || abs(p->pdgId()) == 6 || abs(p->pdgId()) == 5);
    ...
    mcParentage_.push_back(particleHistory.hasLeptonParent() * 16 + particleHistory.hasBosonParent() * 8 +
                           particleHistory.hasNonPromptParent() * 4 + particleHistory.hasQCDParent() * 2 +
...
float ggHiNtuplizer::getGenCalIso(edm::Handle<std::vector<reco::GenParticle>>& handle, reco::GenParticleCollection::const_iterator thisPart,
  ...
  for (auto p = handle->begin(); p != handle->end(); ++p) {
    ...
    if (thisPart->collisionId() != p->collisionId()) continue;
...
float ggHiNtuplizer::getGenTrkIso(edm::Handle<std::vector<reco::GenParticle>>& handle, reco::GenParticleCollection::const_iterator thisPart,
                                  float dRMax) {
...
void ggHiNtuplizer::fillElectrons(const edm::Event& e, const edm::EventSetup& es, reco::Vertex& pv) {
  ...
  for (auto ele = gsfElectronsHandle->begin(); ele != gsfElectronsHandle->end(); ++ele) {
    ...
    if (doEffectiveAreas_) {
      ...
      elePFRelIsoWithEA_.push_back((pfIso.sumChargedHadronPt + std::max(0.0, pfIso.sumNeutralHadronEt + pfIso.sumPhotonEt - rho_ * area)) /
                                   ele->pt());
      elePFRelIsoWithDBeta_.push_back(
          (pfIso.sumChargedHadronPt + std::max(0.0, pfIso.sumNeutralHadronEt + pfIso.sumPhotonEt - 0.5 * pfIso.sumPUPt)) / ele->pt());
      eleEffAreaTimesRho_.push_back(area * rho_);
    }

    bool passConvVeto = !ConversionTools::hasMatchedConversion(*ele, conversions, theBeamSpot->position());
    ...
    if (doEleEReg_) {
      auto sch =
          std::make_unique<SuperClusterHelper>(&(*ele), ele->isEB() ? recHitsEBHandle.product() : recHitsEEHandle.product(), topo, geo);
    ...
    if (doVID_) {
      const edm::Ptr<reco::GsfElectron> elePtr(
          gsfElectronsHandle, ele - gsfElectronsHandle->begin());  // value map is keyed of edm::Ptrs so we need to make one
...
void ggHiNtuplizer::fillPhotons(const edm::Event& e, const edm::EventSetup& es, reco::Vertex& pv) {
  ...
  for (auto pho = recoPhotonsHandle->begin(); pho != recoPhotonsHandle->end(); ++pho) {
    ...
    if (doRecHitsEB_ || doRecHitsEE_) {
      ...
      for (reco::CaloCluster_iterator bc = pho->superCluster()->clustersBegin(); bc != pho->superCluster()->clustersEnd(); ++bc) {
        for (const auto& hitBC : (*bc)->hitsAndFractions()) {
          ...
          math::XYZPoint posXYZ((geo->getPosition(rhDetId)).x() - pv.x(), (geo->getPosition(rhDetId)).y() - pv.y(),
          ...
          for (unsigned int iFlag = 0; iFlag < 32; ++iFlag) {
            if (rh.checkFlag(iFlag)) flagTmp |= 1 << iFlag;
          }
    ...
    if (doPfIso_) {
      ...
      pfpIso1_.push_back(
          pfIso.getPfIso(*pho, reco::PFCandidate::gamma, 0.1, 0.0, 0.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));
      pfpIso2_.push_back(
          pfIso.getPfIso(*pho, reco::PFCandidate::gamma, 0.2, 0.0, 0.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));
      pfpIso3_.push_back(
          pfIso.getPfIso(*pho, reco::PFCandidate::gamma, 0.3, 0.0, 0.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));
      pfpIso4_.push_back(
          pfIso.getPfIso(*pho, reco::PFCandidate::gamma, 0.4, 0.0, 0.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));
      pfpIso5_.push_back(
          pfIso.getPfIso(*pho, reco::PFCandidate::gamma, 0.5, 0.0, 0.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));
      ...
      pfcIso1subUE_.push_back(
          pfIso.getPfIsoSubUE(*pho, reco::PFCandidate::h, 0.1, 0.0, 0.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));
      pfcIso2subUE_.push_back(
          pfIso.getPfIsoSubUE(*pho, reco::PFCandidate::h, 0.2, 0.0, 0.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));
      pfcIso3subUE_.push_back(
          pfIso.getPfIsoSubUE(*pho, reco::PFCandidate::h, 0.3, 0.0, 0.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));
      pfcIso4subUE_.push_back(
          pfIso.getPfIsoSubUE(*pho, reco::PFCandidate::h, 0.4, 0.0, 0.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));
      pfcIso5subUE_.push_back(
          pfIso.getPfIsoSubUE(*pho, reco::PFCandidate::h, 0.5, 0.0, 0.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));

      pfpIso1subUE_.push_back(
          pfIso.getPfIsoSubUE(*pho, reco::PFCandidate::gamma, 0.1, 0.0, 0.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));
      pfpIso2subUE_.push_back(
          pfIso.getPfIsoSubUE(*pho, reco::PFCandidate::gamma, 0.2, 0.0, 0.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));
      pfpIso3subUE_.push_back(
          pfIso.getPfIsoSubUE(*pho, reco::PFCandidate::gamma, 0.3, 0.0, 0.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));
      pfpIso4subUE_.push_back(
          pfIso.getPfIsoSubUE(*pho, reco::PFCandidate::gamma, 0.4, 0.0, 0.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));
      pfpIso5subUE_.push_back(
          pfIso.getPfIsoSubUE(*pho, reco::PFCandidate::gamma, 0.5, 0.0, 0.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));

      pfnIso1subUE_.push_back(
          pfIso.getPfIsoSubUE(*pho, reco::PFCandidate::h0, 0.1, 0.0, 0.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));
      pfnIso2subUE_.push_back(
          pfIso.getPfIsoSubUE(*pho, reco::PFCandidate::h0, 0.2, 0.0, 0.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));
      pfnIso3subUE_.push_back(
          pfIso.getPfIsoSubUE(*pho, reco::PFCandidate::h0, 0.3, 0.0, 0.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));
      pfnIso4subUE_.push_back(
          pfIso.getPfIsoSubUE(*pho, reco::PFCandidate::h0, 0.4, 0.0, 0.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));
      pfnIso5subUE_.push_back(
          pfIso.getPfIsoSubUE(*pho, reco::PFCandidate::h0, 0.5, 0.0, 0.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));
      ...
      pfcIso1pTgt1p0subUE_.push_back(
          pfIso.getPfIsoSubUE(*pho, reco::PFCandidate::h, 0.1, 0.0, 1.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));
      pfcIso2pTgt1p0subUE_.push_back(
          pfIso.getPfIsoSubUE(*pho, reco::PFCandidate::h, 0.2, 0.0, 1.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));
      pfcIso3pTgt1p0subUE_.push_back(
          pfIso.getPfIsoSubUE(*pho, reco::PFCandidate::h, 0.3, 0.0, 1.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));
      pfcIso4pTgt1p0subUE_.push_back(
          pfIso.getPfIsoSubUE(*pho, reco::PFCandidate::h, 0.4, 0.0, 1.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));
      pfcIso5pTgt1p0subUE_.push_back(
          pfIso.getPfIsoSubUE(*pho, reco::PFCandidate::h, 0.5, 0.0, 1.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));

      pfcIso1pTgt2p0subUE_.push_back(
          pfIso.getPfIsoSubUE(*pho, reco::PFCandidate::h, 0.1, 0.0, 2.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));
      pfcIso2pTgt2p0subUE_.push_back(
          pfIso.getPfIsoSubUE(*pho, reco::PFCandidate::h, 0.2, 0.0, 2.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));
      pfcIso3pTgt2p0subUE_.push_back(
          pfIso.getPfIsoSubUE(*pho, reco::PFCandidate::h, 0.3, 0.0, 2.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));
      pfcIso4pTgt2p0subUE_.push_back(
          pfIso.getPfIsoSubUE(*pho, reco::PFCandidate::h, 0.4, 0.0, 2.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));
      pfcIso5pTgt2p0subUE_.push_back(
          pfIso.getPfIsoSubUE(*pho, reco::PFCandidate::h, 0.5, 0.0, 2.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));

      pfcIso1pTgt3p0subUE_.push_back(
          pfIso.getPfIsoSubUE(*pho, reco::PFCandidate::h, 0.1, 0.0, 3.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));
      pfcIso2pTgt3p0subUE_.push_back(
          pfIso.getPfIsoSubUE(*pho, reco::PFCandidate::h, 0.2, 0.0, 3.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));
      pfcIso3pTgt3p0subUE_.push_back(
          pfIso.getPfIsoSubUE(*pho, reco::PFCandidate::h, 0.3, 0.0, 3.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));
      pfcIso4pTgt3p0subUE_.push_back(
          pfIso.getPfIsoSubUE(*pho, reco::PFCandidate::h, 0.4, 0.0, 3.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));
      pfcIso5pTgt3p0subUE_.push_back(
          pfIso.getPfIsoSubUE(*pho, reco::PFCandidate::h, 0.5, 0.0, 3.0, 0, pfIsoCalculator::removePFcand, particlesInIsoMap));
    }
    ...
    if (doGenParticles_ && !isData_) {
      ...
      if (!gpTemp) {
        currentMaxPt = -1;
        for (unsigned igen = 0; igen < mcEt_.size(); ++igen) {
          if (mcStatus_[igen] != 1 || find(otherPdgIds_.begin(), otherPdgIds_.end(), std::abs(mcPID_[igen])) == otherPdgIds_.end())
            continue;
          if (reco::deltaR2(pho->eta(), pho->phi(), mcEta_[igen], mcPhi_[igen]) < delta2 && mcPt_[igen] > currentMaxPt) {
            gpTemp = true;
            currentMaxPt = mcPt_[igen];
