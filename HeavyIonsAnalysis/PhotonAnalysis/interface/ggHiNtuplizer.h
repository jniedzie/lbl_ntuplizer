...
class ggHiNtuplizer : public edm::EDAnalyzer {
 ...
 private:
  ...
  void fillStandaloneMuons(const edm::Event&, const edm::EventSetup&, reco::Vertex& pv);
  ...
  // Et and pT sums
  float getGenCalIso(edm::Handle<std::vector<reco::GenParticle>>&, reco::GenParticleCollection::const_iterator, float dRMax, bool removeMu,
                     bool removeNu);
  ...
  edm::EDGetTokenT<edm::View<reco::Track>> standaloneMuonsCollection_;
  ...
  // reco::Muon
  Int_t nMuStandalone_;
  std::vector<float> muStandalonePt_;
  std::vector<float> muStandaloneEta_;
  std::vector<float> muStandalonePhi_;
  std::vector<int> muStandaloneCharge_;
  std::vector<int> muStandaloneType_;
  std::vector<int> muStandaloneIsGood_;
